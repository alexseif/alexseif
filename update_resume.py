...
CV_MD_PATH = ROOT_DIR / "cv.md"

def build_resume():
    frontmatter = """---
name: "Eskandar (Alex) Seif"
title: "Senior Full-Stack Engineer | Systems Architecture"
...
* **Continuous Specialization:** Low-level LLM architecture (*AI Engineering from Scratch*), agentic CLI engineering (Antigravity), and open-source data pipelines (`php-jobspy`, `freeworld-job-finder`)."""
    ...
    full_cv_content = f"{cv_header}\n\n---\n\n{summary_text}\n\n---\n\n{tech_stack_text}\n\n---\n\n{experience_text}\n\n---\n\n{credentials_text}\n"

    with open(RESUME_MD_PATH, "w", encoding="utf-8") as f:
        f.write(full_resume_content)

    with open(CV_MD_PATH, "w", encoding="utf-8") as f:
        f.write(full_cv_content)

    print("Resume.md and cv.md successfully built in exact chronological order with Dutch HSM formatting.")
