...
LOG_FILE="hsm_conversion.log"
EXPORT_DIR="hsm_exports"
MAX_PARALLEL=4
...
echo "============================================================"
echo "INITIATING HSM CV COMPILER PIPELINE (PARALLEL - ROOT DIRECTORY)"
...
if [ ${#YAML_FILES[@]} -eq 0 ]; then
    echo "[ERROR] No YAML files found in root directory. Halting execution."
    exit 1
fi
...
process_file() {
    ...
    if [ -f "selected/${BASENAME}.yaml" ]; then
        echo "[$(date +'%H:%M:%S')] SKIPPING (exists in selected/): $file"
        return 0
    fi
